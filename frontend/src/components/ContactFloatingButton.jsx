...
/**
 * Mobile-only floating "Contact Us" button.
 * Popup renders ONLY <ContactForm /> — no address, no title, no info cards.
 *
 * - Hidden on desktop (>= lg breakpoint)
 * - Fixed bottom-right, high z-index
 * - Closes via X button, overlay click, or Escape key
      ...
      {open && (
        ...
        >
          ...
          <motion.div
            ...
            exit={{ opacity: 0, y: 30, scale: 0.98 }}
            transition={{ type: "spring", stiffness: 300, damping: 30 }}
            className="absolute inset-x-3 top-4 bottom-4 rounded-3xl overflow-hidden border border-white/10 shadow-2xl bg-[#071120]/90 backdrop-blur-2xl flex flex-col"
          >
            {/* Gradient glow */}
            <div className="pointer-events-none absolute -top-24 -right-24 w-[26rem] h-[26rem] rounded-full bg-gradient-to-br from-[#06B6D4]/25 via-[#3B82F6]/15 to-[#7C3AED]/25 blur-3xl" />
            ...
            <button
              ...
              aria-label="Close"
              data-testid="contact-popup-close"
              className="absolute top-3 right-3 z-10 w-10 h-10 grid place-items-center rounded-full border border-white/10 bg-[#071120]/70 backdrop-blur-md text-white/80 hover:text-white hover:bg-white/10 transition"
            >
              <X className="w-5 h-5" />
            </button>
            ...
            <div
              className="relative flex-1 overflow-y-auto overscroll-contain px-5 py-6 pt-14"
              data-testid="contact-popup-body"
            >
              <ContactForm />
            </div>
